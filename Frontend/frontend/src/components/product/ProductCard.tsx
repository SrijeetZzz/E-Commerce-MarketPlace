    ...
    >
      <CardContent className="p-0 relative">
        ...
        <div className="absolute top-3 left-3 z-10 flex flex-col gap-2">
          ...
          <Badge className="bg-red-500 hover:bg-red-600 text-[10px] font-black px-2 py-0.5 rounded-full border-none shadow-lg">
            20% OFF
          </Badge>

          {/* Sellers Count Badge */}
          {product.listings?.length && product.listings.length > 1 ? (
            <Badge className="bg-slate-900/80 backdrop-blur-sm text-white text-[9px] font-bold border-none shadow-sm rounded-full px-2">
        ...
        <div className="p-4 space-y-2">
          ...
          <h2 className="font-bold text-sm line-clamp-2 text-slate-800 min-h-10 leading-tight group-hover:text-slate-600 transition-colors">
            {product.title}
          </h2>

          {/* Pricing Section */}
          <div
            className="pt-1 flex items-baseline gap-2"
      ...
      <CardFooter className="p-4 pt-0">
        <Button
          ...
          className="w-full bg-slate-900 hover:bg-slate-800 text-white rounded-xl h-11 font-bold text-xs gap-2 transition-all active:scale-95 shadow-xl shadow-slate-100"
        >
